...
namespace HannesTheDev\BroadCastUI;
        ...
        switch ($command->getName()) {
            case "broadcast":
                if ($sender instanceof Player) {
                    if ($sender->hasPermission("broadcastui.broadcast.cmd")) {
                        $this->openBroadCastUI($sender);
                    } else {
                        $sender->sendMessage("§8[§6§lBC§r§8] §r§cYou don't have Permission to use this Command!");
                    }
                } else {
                    $sender->sendMessage("§8[§6§lBC§r§8] §r§r§cYou must be a Player to use this Command!");
                }
                break;
        }
        return true;
    ...
    public function openBroadCastUI(Player $player)
    {
        $form = new CustomForm(function (Player $player, $data = null) {
            if (!empty($data[2])) {
                ...
                $player->sendMessage("§8[§6§lBC§r§8] §r§cPlease enter something in the lines!");
            }
        });
